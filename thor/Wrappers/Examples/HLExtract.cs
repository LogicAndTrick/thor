...
public sealed class HLExtract
{
    ...
    static int _Main(string[] args)
    {
        ...
        uint uiPackage = HLLib.IdInvalid;
        uint uiMode = (uint)HLLib.FileMode.ModeInvalid;

        if(HLLib.hlGetUnsignedInteger(HLLib.Option.Version) < HLLib.VersionNumber)
        {
	        Console.WriteLine("Wrong HLLib version: v{0}.", HLLib.hlGetString(HLLib.Option.Version));
	        return 1;
        }
        ...
        HLLib.ExtractItemStartProc HLExtractItemStartProc = new HLLib.ExtractItemStartProc(ExtractItemStartCallback);
        HLLib.ExtractItemEndProc HLExtractItemEndProc = new HLLib.ExtractItemEndProc(ExtractItemEndCallback);
        HLLib.ExtractFileProgressProc HLExtractFileProgressProc = new HLLib.ExtractFileProgressProc(FileProgressCallback);
        HLLib.ValidateFileProgressProc HLValidateFileProgressProc = new HLLib.ValidateFileProgressProc(FileProgressCallback);
        HLLib.DefragmentFileProgressProc HLDefragmentFileProgressProc = new HLLib.DefragmentFileProgressProc(DefragmentProgressCallback);

        HLLib.hlSetBoolean(HLLib.Option.OverwriteFiles, bOverwriteFiles);
        HLLib.hlSetVoid(HLLib.Option.ProcExtractItemStart, Marshal.GetFunctionPointerForDelegate(HLExtractItemStartProc));
        HLLib.hlSetVoid(HLLib.Option.ProcExtractItemEnd, Marshal.GetFunctionPointerForDelegate(HLExtractItemEndProc));
        HLLib.hlSetVoid(HLLib.Option.ProcExtractFileProgress, Marshal.GetFunctionPointerForDelegate(HLExtractFileProgressProc));
        HLLib.hlSetVoid(HLLib.Option.ProcValidateFileProgress, Marshal.GetFunctionPointerForDelegate(HLValidateFileProgressProc));
        HLLib.hlSetVoid(HLLib.Option.ProcDefragmentProgress, Marshal.GetFunctionPointerForDelegate(HLDefragmentFileProgressProc));
        ...
        if(ePackageType == HLLib.PackageType.None && File.Exists(sPackage))
        {
        ...
        if(ePackageType == HLLib.PackageType.None)
        {
        ...
        {
            Console.WriteLine("Error loading {0}:", sPackage);
            Console.WriteLine(HLLib.hlGetString(HLLib.Option.ErrorShortFormated));
        ...
        uiMode = (uint)HLLib.FileMode.Read;
        uiMode |= !bFileMapping ? (uint)HLLib.FileMode.NoFilemapping : 0;
        uiMode |= bQuickFileMapping ? (uint)HLLib.FileMode.QuickFilemapping : 0;
        uiMode |= bVolatileAccess ? (uint)HLLib.FileMode.Volatile : 0;
        uiMode |= bWriteAccess ? (uint)HLLib.FileMode.Write : 0;
        ...
        if(ePackageType == HLLib.PackageType.Ncf && sNCFRootPath.Length > 0)
        {
    ...
    static void PrintUsage()
    {
        ...
        Console.WriteLine("HLExtract.Net v{0}.{1}.{2} using HLLib v{3}", Name.Version.Major, Name.Version.Minor, Name.Version.Build, HLLib.hlGetString(HLLib.Option.Version));
        Console.WriteLine();
...
#region Progress Callbacks
    ...
    static void ProgressUpdate(uint uiBytesDone, uint uiBytesTotal)
    {
        if(!bSilent)
        {
	        uint uiProgress = uiBytesTotal == 0 ? 100 : (uint)((UInt64)uiBytesDone * 100 / (UInt64)uiBytesTotal);
    ...
    static void ExtractItemStartCallback(int iItem)
    {
        ...
        if(!bSilent)
        {
	        if(HLLib.hlItemGetType(pItem) == HLLib.DirectoryItemType.File)
	        {
    ...
    {
        ProgressUpdate(uiBytesExtracted, uiBytesTotal);
    }
        ...
        else
        {
	        if(!bSilent)
	        {
		        if(HLLib.hlItemGetType(pItem) == HLLib.DirectoryItemType.File)
		        {
			        ...
			        Console.WriteLine("    {0}", HLLib.hlGetString(HLLib.Option.ErrorShortFormated));
		        }
		        else
		        {
    ...
    static void DefragmentProgressCallback(int iFile, uint uiFilesDefragmented, uint uiFilesTotal, uint uiBytesDefragmented, uint uiBytesTotal, ref bool pCancel)
    {
    ...
    static string GetValidation(HLLib.Validation eValidation)
    {
        if(eValidation >= HLLib.Validation.Ok && eValidation <= HLLib.Validation.Error)
        {
    ...
    static HLLib.Validation Validate(IntPtr pItem)
    {
        HLLib.Validation eValidation = HLLib.Validation.Ok, eTest;

        switch(HLLib.hlItemGetType(pItem))
        {
        case HLLib.DirectoryItemType.Folder:
	        if(!bSilent)
	        {
	        ...
	        break;
        case HLLib.DirectoryItemType.File:
	        if(!bSilent)
	        {
	        ...
	        if(bSilent)
	        {
		        switch(eValidation)
		        {
		        case HLLib.Validation.Incomplete:
		        case HLLib.Validation.Corrupt:
    ...
    static void EnterConsole(uint uiPackage, List<string> Commands)
    {
        ...
        while(true)
        {
	        ...
	        if(String.Equals(sCommand, "dir", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        if(sArgument.Length == 0)
		        {
			        ...
			        for(uint i = 0; i < uiItemCount; i++)
			        {
				        ...
				        if(HLLib.hlItemGetType(pSubItem) == HLLib.DirectoryItemType.Folder)
				        {
					        uiFolderCount++;
					        Console.WriteLine("  <{0}>", HLLib.hlItemGetName(pSubItem));
				        }
				        else if(HLLib.hlItemGetType(pSubItem) == HLLib.DirectoryItemType.File)
				        {
					        uiFileCount++;
					        Console.WriteLine("  {0}", HLLib.hlItemGetName(pSubItem));
				        }
		        ...
		        {
			        IntPtr pSubItem = HLLib.hlFolderFindFirst(pItem, sArgument, HLLib.FindType.All | HLLib.FindType.NoRecurse);
			        while(pSubItem != IntPtr.Zero)
			        {
				        ...
				        pSubItem = HLLib.hlFolderFindNext(pItem, pSubItem, sArgument, HLLib.FindType.All | HLLib.FindType.NoRecurse);
			        }
		        }
	        ...
	        else if(String.Equals(sCommand, "cd", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        else
		        {
			        ...
			        else
			        {
				        ...
				        {
					        IntPtr pSubItem = HLLib.hlFolderGetItem(pItem, i);
					        if(HLLib.hlItemGetType(pSubItem) == HLLib.DirectoryItemType.Folder && String.Equals(sArgument, HLLib.hlItemGetName(pSubItem), StringComparison.CurrentCultureIgnoreCase))
					        {
						        bFound = true;
						        pItem = pSubItem;
						        break;
	        ...
	        else if(String.Equals(sCommand, "info", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        {
			        IntPtr pSubItem = HLLib.hlFolderGetItemByPath(pItem, sArgument, HLLib.FindType.All);
			        ...
			        if(pSubItem != IntPtr.Zero)
			        {
				        ...
				        case HLLib.DirectoryItemType.Folder:
					        ...
					        Console.WriteLine("  Size: {0} B", HLLib.hlFolderGetSize(pSubItem, true));
					        Console.WriteLine("  Size On Disk: {0} B", HLLib.hlFolderGetSizeOnDisk(pSubItem, true));
				        ...
				        {
                            HLLib.Attribute Attribute;
					        if(HLLib.hlPackageGetItemAttribute(pSubItem, (HLLib.PackageAttribute)i, out Attribute))
					        {
                                if(Attribute.eAttributeType != HLLib.AttributeType.Invalid)
                                {
	        ...
	        else if(String.Equals(sCommand, "extract", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        else
		        {
			        ...
			        {
				        pSubItem = HLLib.hlFolderGetItemByName(pItem, sArgument, HLLib.FindType.All);
			        }
	        ...
	        else if(String.Equals(sCommand, "defragment", StringComparison.CurrentCultureIgnoreCase))
	        {
                ...
                HLLib.hlSetBoolean(HLLib.Option.ForceDefragment, bForceDefragment);

                Console.Write("  Progress: ");
                ProgressStart();
                if(!HLLib.hlPackageDefragment())
                {
                    Console.Write(" {0}", HLLib.hlGetString(HLLib.Option.ErrorShortFormated));
                }
	        ...
	        else if(String.Equals(sCommand, "find", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        else
		        {
			        ...
			        uint uiFolderCount = 0, uiFileCount = 0;
			        IntPtr pSubItem = HLLib.hlFolderFindFirst(pItem, sArgument, HLLib.FindType.All);
			        while(pSubItem != IntPtr.Zero)
			        {
                        switch(HLLib.hlItemGetType(pSubItem))
                        {
				        ...
				        pSubItem = HLLib.hlFolderFindNext(pItem, pSubItem, sArgument, HLLib.FindType.All);
			        }
	        ...
	        else if(String.Equals(sCommand, "type", StringComparison.CurrentCultureIgnoreCase))
	        {
		        ...
		        {
			        IntPtr pSubItem = HLLib.hlFolderGetItemByName(pItem, sArgument, HLLib.FindType.Files);
		        ...
		        else
		        {
			        ...
			        if(pSubItem != IntPtr.Zero)
			        {
				        ...
				        if(HLLib.hlFileCreateStream(pSubItem, out pStream))
				        {
					        if(HLLib.hlStreamOpen(pStream, (uint)HLLib.FileMode.Read))
					        {
						        HLLib.PackageType ePackageType = HLLib.hlGetPackageTypeFromStream(pStream);

                                uint uiSubPackage;
						        if(HLLib.hlCreatePackage(ePackageType, out uiSubPackage))
						        {
							        HLLib.hlBindPackage(uiSubPackage);
							        if(HLLib.hlPackageOpenStream(pStream, (uint)HLLib.FileMode.Read))
							        {
								        if(!bSilent)
	        ...
	        {
		        Console.WriteLine("Total size: {0} B", HLLib.hlGetUnsignedInteger(HLLib.Option.PackageSize));
		        Console.WriteLine("Total mapping allocations: {0}", HLLib.hlGetUnsignedInteger(HLLib.Option.PackageTotalAllocations));
		        Console.WriteLine("Total mapping memory allocated: {0} B", HLLib.hlGetUnsignedInteger(HLLib.Option.PackageTotalMemoryAllocated));
		        Console.WriteLine("Total mapping memory used: {0} B", HLLib.hlGetUnsignedInteger(HLLib.Option.PackageTotalMemoryUsed));

		        uint uiAttributeCount = HLLib.hlPackageGetAttributeCount();
		        for(uint i = 0; i < uiAttributeCount; i++)
		        {
                    HLLib.Attribute Attribute;
			        if(HLLib.hlPackageGetAttribute((HLLib.PackageAttribute)i, out Attribute))
			        {
                        if(Attribute.eAttributeType != HLLib.AttributeType.Invalid)
                        {
                            Console.WriteLine("{0}: {1}", Attribute.GetName(), Attribute.ToString());
